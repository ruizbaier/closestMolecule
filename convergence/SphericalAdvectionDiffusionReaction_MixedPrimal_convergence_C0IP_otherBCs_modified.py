...
There is an issue with setting up a Dirichlet condition on the "outflow boundary" because the upstream mechanism does
not work. Burman-He 2019 proposes to impose the Dirichlet on the outflow weakly. This can be done as in the code or
with, e.g.

LHS += (h*min(0,rvec.n)^2 + \gamma*eps^2/h)*q*w * ds(right)
RHS += (h*min(0,rvec.n)^2 + \gamma*eps^2/h)*q_ex*w * ds(right)
...
for nk in range(nkmax):
    ...
    lhs = dot(sig + G(p, q), tau) * weight * dx \
          ...
          + stab / (deg + 1) ** 3.5 * avg(hK) ** 2 * beta * dot(jump(grad(q)), n('+')) * dot(jump(grad(w)),
                                                                                             n('+')) * weight * dS
    ...
    q_h.rename("q", "q")
    fileO.write(q_h, 1.0 * nk)
    p_h.rename("p", "p")
    fileO.write(p_h, 1.0 * nk)
    sig_h.rename("s", "s")
    fileO.write(sig_h, 1.0 * nk)
    # ********* Computing errors ****** #
...
print('========================================================================')
for nk in range(nkmax):
    print('{:6d} & {:.4f} & {:6.2e} & {:.3f} & {:6.2e} & {:.3f} & {:6.2e} & {:.3f} '.format(nn[nk], hh[nk], es[nk],
                                                                                            rs[nk], ep[nk], rp[nk],
                                                                                            eq[nk], rq[nk]))
print('========================================================================')