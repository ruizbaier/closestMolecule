...
s_left = Expression(("0", "0"), degree=deg, c=c, V=V, o=sigma, domain=mesh)
...
flux_steady_state = Expression(("0", "-(c/V)*exp(-(4/3)*pi*c*pow(x[0],3))*(o/pow(x[1],2))"), degree=deg, o=sigma,  c=c,
                               V=V, domain=mesh)
...
while (t <=tfinal):
    ...
    t += dt
computed_flux = assemble(dot(dMatrix*project(-grad(p_h), FunctionSpace(mesh, "RT", deg)), n)*weight*ds(bottom))
total_flux = assemble(dot(dMatrix*s_h, n)*weight*ds(bottom))
print(f'Total flux: {total_flux} computed: {computed_flux}')