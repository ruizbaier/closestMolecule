...
# output file
output_file = XDMFFile("outputs/flat_solutions1.xdmf")
...
u = interpolate(Expression(("f*(c/V)*exp(-(4/3)*pi*c*pow(x[0],3))*(1-o/x[1])", "0","0","f*(1/(4*pi*V))*exp(-4/3*pi*c*pow(x[0],3))*(1-o/x[1])"), degree=deg, c=c, V=V, o=sigma, f=factor, domain=mesh), mixed_space)
du = TrialFunction(mixed_space)
p, s, q = split(u)

# ********* initial and boundary conditions ******** #
p_old = interpolate(Expression("f*(c/V)*exp(-(4/3)*pi*c*pow(x[0],3))*(1-o/x[1])", degree=deg, c=c, V=V,
                                           o=sigma, f=factor, domain=mesh),mixed_space.sub(0).collapse())

p_right = Expression("f*(c/V)*exp(-(4/3)*pi*c*pow(x[0],3))*(1 - o/x[1])", degree=deg, c=c, V=V, o=sigma, f=factor, domain = mesh)
p_top = Expression("f*(c/V)*exp(-(4/3)*pi*c*pow(x[0],3))*(1 - o/x[1])", degree=deg, c=c, V=V, o=sigma, f=factor, domain=mesh)
p_bottom = Constant(0.)
...
q_right = Expression("f/(4*pi*V)*exp(-(4/3)*pi*c*pow(x[0],3))*(1 - o/x[1])", degree=deg, c=c, V=V, o=sigma, f=factor, domain=mesh)
...
s_left = Expression(("0", "-f*(c/V)*exp(-(4/3)*pi*c*pow(x[0],3))*(o/pow(x[1],2))"), degree=deg, c=c, V=V, o=sigma, f=factor, domain=mesh)
...
p_steady_state = Expression("f*c/V*exp(-4/3*pi*c*pow(x[0],3))*(1-o/x[1])", degree=deg-1, o=sigma,  c=c, V=V, f=factor, domain=mesh)
flux_steady_state = Expression(("0", "0"), degree=deg, o=sigma, c=c,
                               V=V, f=factor, domain=mesh)

p_approx = interpolate(p_steady_state, mixed_space.sub(0).collapse())
flux_approx = interpolate(flux_steady_state, mixed_space.sub(1).collapse())
...
while (t <=tfinal):
    ...
    dif_flux = project((s_h - flux_approx), mixed_space.sub(1).collapse())
...
r1_point = 0.25
spacing = 0.01
r2_points = np.arange(0, 1+spacing, spacing)
sum = 0
for r2_point in r2_points:
    # Get flux
    flux = s_h(Point(r2_point, r1_point))[1]
    sum += flux*(4*np.pi*r1_point*r2_point)**2*-D1
print(f'Total flux: {total_flux} expected: {4*np.pi*D1*sigma} flux numpy: {sum*spacing}')
